...
import { THREEWingGeometry, THREEPropGeometry, getMeshCount, try_set_parent, update_prop_controls, setMeshCount } from "./MU.js";
...
function MenubarAdd( editor ) {
	...
	option.onClick( function () {
		...
		const parameters = tempObject?.geometry?.parameters || wing_defaults;

		var geometry = new THREEWingGeometry( parameters );
		var material = new THREE.MeshPhongMaterial();
		var mesh = new THREE.Mesh( geometry, material );
		const newMeshCount = getMeshCount() + 1;
		mesh.name = 'Wing_' + ( newMeshCount );
	...
	option.onClick( function () {
		...
		var geometry = new THREEPropGeometry( nblades, rotation, diameter, pitch, hub_radius, root_chord, tip_chord, elliptical, root_airfoil, tip_airfoil, electric_motor, nSeg, nAFseg, same_as_root );
		var material = new THREE.MeshPhongMaterial( { side: 2 } );
		var mesh = new THREE.Mesh( geometry, material );
		const newMeshCount = getMeshCount() + 1;
		mesh.name = 'Prop_' + ( newMeshCount );
		setMeshCount( newMeshCount );
		editor.execute( new AddObjectCommand( editor, mesh ) );
		editor.select( mesh ); // DFH
		if ( tempObject !== null ) try_set_parent( editor, mesh, tempObject );
